...
<script>
	...
	export default {
		...
		computed: {
			...
			locale() {
				return this.user.preferences.global.lang;
			},
	    	avatarId() {
	        ...
	        paginationUrl() {
	        	return '/administration/users/' + this.profileUser.id + '?page=';
	        }
	    },
		...
		methods: {
			deleteAvatar() {
	            axios.delete('/core/avatars/' + this.user.avatarId).then(response => {
	        ...
	        logout() {
	        	axios.post('/logout').then(() => {
	        		window.location.href = '/login'; //fixme $router ;)
	        	});
	        },
	        getDay(timestamp) {
	        ...
	        getHRDate(timestamp) {
	        	return moment(timestamp).format(this.$store.state.dateFormat);
	        },
	        getHRTime(timestamp) {
